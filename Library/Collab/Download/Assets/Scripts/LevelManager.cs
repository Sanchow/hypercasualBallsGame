...
public class LevelManager : MonoBehaviour
{
    ...
    public float LevelProgress{ 
        get{
            ...
            return currentBlocksHP/totalBlocksHP;
        }
    }
    ...
    public void CheckIfAnyBlocksAreLeft(){
        if(instantiatedBlocks.Count == 0 && rowCounter == levelData.Count){
    ...
    public void ChangeState(LevelState newState){
        if(CurrentLevelState != newState){
            HandleLevelStateChanged(CurrentLevelState, newState);
    ...
    void HandleLevelStateChanged(LevelState oldState, LevelState newState){
