...
public class TrajectoryRenderer : MonoBehaviour
{
    ...
    public void SetTrajectoryPoints(Vector2 targetPosition)
    {
        ...
        trajectoryPoints.Add(renderStartTransform.position);

        Vector2 direction = targetPosition - new Vector2(renderStartTransform.position.x, renderStartTransform.position.y);
        direction.Normalize();

        RaycastHit2D hit = Physics2D.Raycast(renderStartTransform.position, direction, Mathf.Infinity, layerMask);

        if(hit.collider != null){
            Vector2 hitPoint = hit.point;
            float distance = Vector2.Distance(hitPoint, renderStartTransform.position);
            numberOfSegmentsBeforeBounce = Mathf.RoundToInt(distance / distanceBetweenEachSegment);
            Vector2 previousPoint = renderStartTransform.position;
            for (int i = 0; i < numberOfSegmentsBeforeBounce; i++)
            {
                Vector2 nextPoint = previousPoint + direction * distanceBetweenEachSegment;
                if(Vector2.Distance(nextPoint, renderStartTransform.position) <= Vector2.Distance(hitPoint, renderStartTransform.position)){
    ...
    public void HandleGameStateChanged(LevelState oldState, LevelState newState){
        if(newState == LevelState.WAITING){
            if(oldState == LevelState.PLAYING || oldState == LevelState.ACCELERATED){
                ClearAllDots();
            }
